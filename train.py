...
def build_model(model_name: str, cfg: dict, n_classes: int):
...
def train_model(model_name: str, cfg: dict, seed: int = None):
    ...
    model, is_quantum = build_model(model_name, cfg, n_classes)
    model.to(device)
    ...
    if is_quantum:
        ...
        param_report = build_param_report(
            model_name, model, model.qlayer.quantum_parameters,
    ...
    if cfg["logging"]["save_checkpoints"]:
        ...
        torch.save(model.state_dict(), results_dir / "checkpoints" / f"{model_name}_seed{seed}.pt")
    ...
    with open(results_dir / "logs" / f"train_{model_name}_seed{seed}.json", "w") as f:
...
def main():
    ...
    _, log, _ = train_model(args.model, cfg, seed=args.seed)
